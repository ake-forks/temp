...
(rf/reg-event-fx ::submit-success
  (fn [{:keys [db]} [_ case-id {:keys [path]} _response]]
    {:db (fork/set-submitting db path false)
     ::ui/navigate [:dashboard {:case-id case-id}]}))
...
(defn deceased-details-form [{:keys [_handle-submit _submitting?]
                              :as fork-args}]
  [:form
...
(defmethod routes/panels :create-deceased-details-panel [] [panel])
